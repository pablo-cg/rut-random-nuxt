...
const rutStore = useRutStore()
const emailStore = useEmailStore()

const { totalRutsCopied, totalRutsGenerated } = storeToRefs(rutStore)
const { totalEmailsCopied, totalEmailsGenerated } = storeToRefs(emailStore)
    ...
    >
      ...
      >
        ...
        <div class="px-2 flex flex-col items-center gap-1" v-if="loading">
          <Icon name="line-md:loading-twotone-loop" size="3rem" />
        </div>
        <ul class="px-2 flex flex-col gap-1" v-else>
          <li>RUTs Generados: {{ totalRutsGenerated }}</li>
          <li>RUTs Copiados: {{ totalRutsCopied }}</li>
          <li>Emails Generados: {{ totalEmailsGenerated }}</li>
          <li>Emails Copiados: {{ totalEmailsCopied }}</li>
        </ul>
      </section>
      <section
        class="border col-span-full border-stone-300 dark:border-stone-600 rounded-lg p-3"
